...
document.querySelectorAll(priorityItem.join(",")).forEach((prioritySubmenuItem) => {
  prioritySubmenuItem.addEventListener("click", () => {
    for (let i = 0; i < priorityItem.length; i++) {
      document.querySelectorAll(priorityItem[i]).forEach((el) => {
        el.classList.remove("priority__submenu-item--active");
      });
    }
    ...
    document.querySelectorAll(`#${prioritySubmenuItem.id}`).forEach((el) => {
      el.classList.add("priority__submenu-item--active");
    });
