...
const sidebarListLinkActive = function (active) {
  const sidebarListLink = [sidebarListAddInputBTN, sidebarListAddTodayBTN, sidebarListAddCompletedBTN, sidebarListFilterLabelBTN];
...
const sectionActive = function (active) {
  const sectionActive = ['input-section', 'today-section', 'completed-section', 'label-filter-section'];
...
sidebarListAddTodayBTN.addEventListener('click', () => {
  titleSection.textContent = `کار های امروز | ${todayDate.format('dddd (DD MMMM  YYYY)')}`;
...
// --- FILTER & LABEL  --------------------------------------------------------
const sidebarListFilterLabelBTN = $.getElementById('sidebar-list__filter-label');
sidebarListFilterLabelBTN.addEventListener('click', () => {
  titleSection.textContent = 'فیلتر ها و برچسب ها';
  sidebarListLinkActive(sidebarListFilterLabelBTN);
  sectionActive('label-filter-section');
});
